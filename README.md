...
1. Config new user, we wont use root account.
   ...
   4. Use `sudo visudo` and add `USER_NAME ALL=(ALL) ALL` to the end of the file to grant a specific user administrative privileges.
...
3. Openssh Hardening
   ...
   1. Remove password authentication on SSH
      ...
      2. Change settings to:
         ...
         4. Set path `AuthorizedKeysFile .ssh/authorized_keys`
      3. Try to exit vim XD or just CTRL+X in nano
...
4. Set the domain
   ...
   1. Buy the domain 🚀
   2. Config DNS, all major hosting services has nice easy one click button implementation process.
   3. To get the ip of the server type `ip address`
...
8. Create Github auto deploy action
   ...
   3. Set envs, go to Settings -> Secrets -> Actions -> New repository secret
   ...
   4. Setup action, in that case were using
   5. Add GitHub hosts to known hosts `ssh-keyscan -H github.com >> ~/.ssh/known_hosts` then `chmod 644 ~/.ssh/known_hosts`
